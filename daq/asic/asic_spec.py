...
from . import helpers
from . import register_space_helpers as reg
from . import config_packet_helpers as pkt
...
class asic_spec:
    ...
    def print_field_reset_values(self):
        ...
        reg.print_register_field_reset_values(self.asic_dict)

    def build_register_write_list(self, update=None, as_needed=None):
        ...
        """
        return reg.build_register_write_list(self.asic_dict, self.field_to_reg, self.reg_to_field, update=update, as_needed=as_needed)

    def build_register_read_list(self, refresh=None):
        ...
        Raises:
            KeyError if an update field is unknown
        """
        return reg.build_register_read_list(self.asic_dict, self.field_to_reg, refresh=refresh)
    ...
    def build_config_write_list(self, chip: int, update=None, as_needed=None) -> list[int]:
        ...
        Calls build_register_write_list and uses the output to
        ...
        """
        reg_list = self.build_register_write_list(update=update, as_needed=as_needed)
    ...
    def build_config_read_list(self, chip: int, refresh=None) -> list[int]:
        ...
        Calls build_register_read_list and uses the output to
