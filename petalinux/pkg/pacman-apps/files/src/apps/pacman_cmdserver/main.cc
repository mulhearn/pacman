...
int main(int argc, char* argv[]){
  ...
  while (1) {
    ...
    size_t nbytes_recv = zmq_msg_recv(&req_msg, rep_socket, 0);
    ...
    static pacman_msg_t msg;
    assert(nbytes_recv <= sizeof(msg));
    memcpy(&msg, zmq_msg_data(&req_msg), nbytes_recv);
    print_msg(&msg, "INFO:  ");
    assert(nbytes_recv == msg.header.n_bytes + HEADER_BYTES);
	...
	(req_str, req_len, reinterpret_cast<char*>(rep_msg.raw), sizeof(rep_msg.raw));
      ...
      rep_msg.header.timestamp_lo  = msg.header.timestamp_lo;
      rep_msg.header.timestamp_hi  = msg.header.timestamp_hi;

      // send reply:
      msg_ready = false;
      static zmq_msg_t zmq_rep;
      zmq_msg_init_data(&zmq_rep, &rep_msg, HEADER_BYTES + rep_len, clear_msg, NULL);
    ...
    write_header_rep(&rep_msg.header, n_bytes, msg.header.timestamp_hi, msg.header.timestamp_lo);
    ...
    zmq_msg_t zmq_rep;
    msg_ready = false;
    zmq_msg_init_data(&zmq_rep, &rep_msg,HEADER_BYTES + n_bytes, clear_msg, NULL);
    zmq_msg_send(&zmq_rep, rep_socket, 0);
    while (!msg_ready) { usleep(1000); }
    zmq_msg_close(&zmq_rep);
  }
