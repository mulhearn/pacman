...
#define BUFFER_LENGTH (N_WORDS * WORD_LENGTH) // bytes

#define SRC  0x38000000
#define DEST 0x39000000
...
int dma_mm2s_sync(unsigned int* dma_virtual_address) {
    ...
    while(!(mm2s_status & (1<<12 | 1<<13)) || !(mm2s_status & 1<<1) ){
        // while not complete / error or busy
...
int dma_s2mm_sync(unsigned int* dma_virtual_address) {
    ...
    while(!(s2mm_status & (1<<12 | 1<<13)) || !(s2mm_status & 1<<1)){
        // while not complete / error or busy
        dma_s2mm_status(dma_virtual_address);
        dma_mm2s_status(dma_virtual_address);
...
void dma_s2mm_status(unsigned int* dma_virtual_address) {
    ...
    if (status & 0x00000002) printf(" idle"); else printf(" busy");
...
int main() {
    ...
    unsigned int* virtual_source_address  = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x38000000); // Memory map source address
    unsigned int* virtual_destination_address = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x39000000); // Memory map destination address

    for (int i=0; i<N_WORDS; i++) {
      virtual_source_address[i] = i; // Write stuff to source block
    }
    memset(virtual_destination_address, 0, BUFFER_LENGTH); // Clear destination block

    printf("Source memory block:\n"); memdump(virtual_source_address, BUFFER_LENGTH);
    printf("Destination memory block:\n"); memdump(virtual_destination_address, BUFFER_LENGTH);
    ...
    printf("Starting S2MM channel with all interrupts enabled...\n");
    dma_set(virtual_address, S2MM_CONTROL_REGISTER, 0xf001);
    dma_s2mm_status(virtual_address);

    printf("Starting MM2S channel with all interrupts enabled...\n");
    dma_set(virtual_address, MM2S_CONTROL_REGISTER, 0xf001);
    dma_mm2s_status(virtual_address);

    printf("Writing S2MM transfer length...\n");
    dma_set(virtual_address, S2MM_LENGTH, BUFFER_LENGTH);
    dma_s2mm_status(virtual_address);

    printf("Writing MM2S transfer length...\n");
    dma_set(virtual_address, MM2S_LENGTH, BUFFER_LENGTH);
    dma_mm2s_status(virtual_address);
    ...
    printf("Clear MM2S IRQ...\n");
    unsigned int mm2s_status = dma_get(virtual_address, MM2S_STATUS_REGISTER);
    dma_set(virtual_address, MM2S_STATUS_REGISTER, mm2s_status);
    dma_mm2s_status(virtual_address);

    printf("Clear S2MM IRQ..\n");
    unsigned int s2mm_status = dma_get(virtual_address, S2MM_STATUS_REGISTER);
    dma_set(virtual_address, S2MM_STATUS_REGISTER, s2mm_status);
    dma_s2mm_status(virtual_address);
