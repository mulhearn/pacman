...
void send_reply(char* reply, uint32_t& reply_bytes, void* rep_socket, void* echo_socket) {
...
int main(int argc, char* argv[]){
  ...
  while (1) {
    ...
    if (*msg_type != '?') {
      char* reply;
      uint32_t reply_bytes = init_msg(reply, 1, MSG_TYPE_REP);
      set_rep_word_err(get_word(reply, 0), NULL, NULL);
      send_reply(reply, reply_bytes, rep_socket, echo_socket);
      zmq_msg_close(req_msg);
      continue;
    // check that received bytes matches expected based on message contents
    } else if (req_msg_nbytes != get_msg_bytes(msg)) {
      char* reply;
      uint32_t reply_bytes = init_msg(reply, 1, MSG_TYPE_REP);
      set_rep_word_err(get_word(reply, 0), NULL, NULL);
      send_reply(reply, reply_bytes, rep_socket, echo_socket);
